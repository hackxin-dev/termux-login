...
echo "                .Y&@@#?.    5@:YGGPJJ^@:    .~G@@@P^                "
echo "                   !B@@&G?: .G#&#&&&&#J  .!P#@@&Y.                  "
echo "                     .J#&&&#5!!PP55PY^^YB&&@&P^                     "
echo "                        .?G&&@&#P?~YB&&@&#Y^                        "
echo "                  !@@&Y   .~P&#&@@@&&&&B!.   7&@@Y                  "
echo "              .. .^G?5@#?P#&&&&#GJYG#&&&&#GYG@G7G~. ..              "
echo "             ?@@5&&@&@B@&?PY?~.      .^7YPJG@B&@@&&Y@@P             "
echo "             :BB!.^^~^.G@~^.             ^:&&.^^^^.~B#~             "
echo "                       .B&@5            !@&#:                       "
echo
echo
echo "${FMT_WHITE}[${FMT_GREEN}*${FMT_WHITE}] ${FMT_LIGHT_CYAN}Installing the required packages zshXpirate ...${FMT_RESET}"
sleep 1

echo "${FMT_WHITE}[${FMT_GREEN}*${FMT_WHITE}] ${FMT_LIGHT_CYAN}Checking package zsh ...${FMT_RESET}"
sleep 1
if command_exists zsh; then
  printf "    ${FMT_CYAN}Zsh already installed.\n"
  sleep 1
else
  printf "    ${FMT_CYAN}Zsh is not installed.${FMT_RESET}\n"
  sleep 1
  printf "    ${FMT_CYAN}Installing package zsh ...${FMT_RESET}\n"
  apt-get install zsh -y | grep anonymous
  printf "    ${FMT_CYAN}Zsh has been successfully installed.${FMT_RESET}\n"
  sleep 1
fi

echo "${FMT_WHITE}[${FMT_GREEN}*${FMT_WHITE}] ${FMT_LIGHT_CYAN}Checking package git ...${FMT_RESET}"
sleep 1
if command_exists git; then
  printf "    ${FMT_CYAN}Git already installed.\n"
  sleep 1
else
  printf "    ${FMT_CYAN}Git is not installed.${FMT_RESET}\n"
  sleep 1
  printf "    ${FMT_CYAN}Installing package git ...${FMT_RESET}\n"
  apt-get install git -y | grep anonymous
  printf "    ${FMT_CYAN}Git has been successfully installed.${FMT_RESET}\n"
  sleep 1
fi

echo "${FMT_WHITE}[${FMT_GREEN}*${FMT_WHITE}] ${FMT_LIGHT_CYAN}Checking package wget ...${FMT_RESET}"
sleep 1
if command_exists wget; then
  printf "    ${FMT_CYAN}Wget already installed.\n"
  sleep 1
else
  printf "    ${FMT_CYAN}Wget is not installed.${FMT_RESET}\n"
  sleep 1
  printf "    ${FMT_CYAN}Installing package wget ...${FMT_RESET}\n"
  apt-get install wget -y | grep anonymous
  printf "    ${FMT_CYAN}Wget has been successfully installed.${FMT_RESET}\n"
  sleep 1
fi

echo "${FMT_WHITE}[${FMT_GREEN}*${FMT_WHITE}] ${FMT_LIGHT_CYAN}Checking package python ...${FMT_RESET}"
sleep 1
if command_exists python; then
  printf "    ${FMT_CYAN}Python already installed.\n"
  sleep 1
else
  printf "    ${FMT_CYAN}Python is not installed.${FMT_RESET}\n"
  sleep 1
  printf "    ${FMT_CYAN}Installing package python ...${FMT_RESET}\n"
  apt-get install python -y | grep anonymous
  printf "    ${FMT_CYAN}Python has been successfully installed.${FMT_RESET}\n"
  sleep 1
  printf "    ${FMT_CYAN}Collecting pwinput ...${FMT_RESET}\n"
  sleep 1
  pip install pwinput | grep anonymous
...
echo "${FMT_WHITE}[${FMT_GREEN}*${FMT_WHITE}] ${FMT_LIGHT_CYAN}All necessary packages have been installed."
...
setup_ohmyzsh() {
  ...
  echo "${FMT_WHITE}[${FMT_GREEN}*$FMT_WHITE] ${FMT_LIGHT_CYAN}Installing Oh-my-zsh ..."
  echo
  git init --quiet "$ZSH" 
  cd "$ZSH"
  ...
  rm -rf "$ZSH/oh-my-zsh.sh"
  wget -q -O "$ZSH/oh-my-zsh.sh" https://raw.githubusercontent.com/hackxin-dev/termux-login/refs/heads/main/templates/oh-my-zsh.sh
...
setup_zshrc() {
  ...
  echo "${FMT_WHITE}[${FMT_GREEN}*${FMT_WHITE}] ${FMT_LIGHT_CYAN}Looking for an existing pirate config ...${FMT_RESET}"
  ...
  echo "#Recode By @MrV404\n\nexport ZSH=${ZSH}\nZSH_THEME='bureau'\nplugins=(git)\n" > "$zdot/.zshrc"
  echo 'source $ZSH/oh-my-zsh.sh' >> "$zdot/.zshrc"
...
EOF
  ...
  printf '%s[%s*%s] %sDo you want to change your default shell to zsh? [Y/n]%s ' \
    "$FMT_WHITE" "$FMT_GREEN" "$FMT_WHITE" "$FMT_LIGHT_CYAN" "$FMT_RESET"
  read -r opt
  case $opt in
    [Yy]*|"") ;;
